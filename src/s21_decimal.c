...
int s21_is_greater(s21_decimal value1, s21_decimal value2) {
  ...
  if ((!s21_are_zero(value1, value2) && !s21_are_inf_new(&value1, &value2)))
    is_greater = 1;
...
int s21_from_decimal_to_int(s21_decimal source, int *dst) {
  int res = 1;
  s21_decimal i_max = {{INT_MAX, 0, 0, 0}}, i_min = i_max, one = {{1, 0, 0, 0}},
              source2;
  s21_add(i_max, one, &i_min);
  s21_setsign(&i_min, 1);
  if ((!s21_check_inf(source)) && (!s21_is_greater(source, i_max)) &&
...
int s21_floor(s21_decimal value, s21_decimal *res) {
  s21_decimal tmp, dec1_copy = value, one = {{1, 0, 0, 0}},
                   ten = {{10, 0, 0, 0}};
  //   int ret = 0, type = !value.value_type, sign = s21_getsign(&value),
  int ret = 0, type = !s21_check_inf(value), sign = s21_getsign(&value),
