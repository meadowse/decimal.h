...
int s21_check_inf(s21_decimal value) {
    int ret = 1;
    if ((s21_get_scale(&value) == 28) && (value.bits[0] == 0) && (value.bits[1] == 0) && (value.bits[0] == 3))
        ret = 0;
    ...
    if (!s21_check_inf(*value2) && s21_check_inf(*value1)) {
        ...
        int s21_div(s21_decimal divident, s21_decimal divisor, s21_decimal * result) {
            ...
            int divsr_is_normal_plus =
                s21_is_greater(divisor, zero) == 1 && divisor.value_type == s21_usual ? 1 : 0;

            int divsr_is_normal_minus =
                s21_is_less(divisor, zero) == 1 && divisor.value_type == s21_usual ? 1 : 0;
