...
s21_decimal s21_check_boundary(s21_decimal value1, s21_decimal value2) {
    ...
    res.value_type = value1.value_type > value2.value_type ? value1.value_type : value2.value_type;
    ...
    if (res->value_type == s21_usual || s21_get_bit(*res, 126)) {
        if (!s21_getsign(&value1) && !s21_getsign(&value2)) {  // positive
            ...
            if (tmp.value_type == s21_infinity && s21_get_scale(&value1) == 0) {
                res->value_type = s21_infinity;
            } else if (tmp.value_type == s21_infinity && s21_get_scale(&value1) > 0) {  // lower scale
...
int s21_are_inf(s21_decimal *value1, s21_decimal *value2) {
    ...
    return res;  // 0 - не бесконечны, 1 - первое бесконечно, 2 - оба бесконечны,
                 // -1 - второе бесконечно
}

int s21_are_neg_inf(s21_decimal *a, s21_decimal *b) {
    int res = 0;
    if (a->value_type == s21_neg_infinity && b->value_type != s21_neg_infinity) res = 1;
    if (a->value_type != s21_neg_infinity && b->value_type == s21_neg_infinity) res = -1;
    if (a->value_type == s21_neg_infinity && b->value_type == s21_neg_infinity) res = 2;
    return res;  // 0 - не бесконечны, 1 - первое бесконечно, 2 - оба бесконечны,
                 // -1 - второе бесконечно
}
...
int s21_are_neg(s21_decimal *value1, s21_decimal *value2) {
    ...
    return res;  // 0 - оба одного знака, 1 - первое положительное, -1 - второе
                 // положительное
}
...
int s21_is_greater(s21_decimal value1, s21_decimal value2) {
    ...
    if (((value1.value_type == s21_nan || value2.value_type == s21_nan)) ||
        (!s21_are_zero(value1, value2) && !s21_are_inf(&value1, &value2) &&
         !s21_are_neg_inf(&value1, &value2)))
        is_greater = 1;

    if (is_greater == -1) {
        // 0 - не бесконечны, 1 - первое бесконечно, 2 - оба бесконечны, -1 - второе
        // бесконечно
...
int s21_from_decimal_to_int(s21_decimal source, int *dst) {
    int res = 1;
    s21_decimal i_max = {{INT_MAX, 0, 0, 0}, 0}, i_min = i_max, one = {{1, 0, 0, 0}, 0}, source2;
    s21_add(i_max, one, &i_min);
    s21_setsign(&i_min, 1);
    if ((source.value_type == 0) && (!s21_is_greater(source, i_max)) && (!s21_is_less(source, i_min))) {
...
s21_decimal check_for_mul(s21_decimal value1, s21_decimal value2) {
    ...
    int there_is_nan = ((value1.value_type == s21_nan || value2.value_type == s21_nan)) ? 1 : 0;
    int there_is_neg_inf = s21_are_neg_inf(&value1, &value2) != 0 ? 1 : 0;
    int there_is_inf = s21_are_inf(&value1, &value2) != 0 ? 1 : 0;
    int there_is_zero = s21_is_equal(value1, zero) == 1 || s21_is_equal(value2, zero) == 1 ? 1 : 0;
    ...
    if (there_is_nan || (there_is_zero && there_is_inf) || (there_is_zero && there_is_neg_inf)) {
        res.value_type = s21_nan;
    } else if ((there_is_plus_normal && there_is_inf) || (there_is_neg_normal && there_is_neg_inf) ||
               (value1.value_type == s21_infinity && value2.value_type == s21_infinity) ||
               (value1.value_type == s21_neg_infinity && value2.value_type == s21_neg_infinity)) {
        res.value_type = s21_infinity;
...
int s21_div(s21_decimal divident, s21_decimal divisor, s21_decimal *result) {
    ...
    if (divident.value_type == s21_nan || divisor.value_type == s21_nan) {
    ...
    } else if (s21_is_equal(divisor, zero) == 1 && s21_is_less(divident, zero) == 1) {
        //    -x/0
        ...
        //    0/0
        result->value_type = s21_nan;
    } else if ((divident.value_type == s21_infinity || divident.value_type == s21_neg_infinity) &&
               (divisor.value_type == s21_infinity || divisor.value_type == s21_neg_infinity)) {
        //    +-inf/(+-inf)
        result->value_type = s21_nan;
    } else if ((divident.value_type == s21_infinity && divsr_is_normal_plus) ||
               (divident.value_type == s21_neg_infinity && divsr_is_normal_minus)) {
        //    +inf / +normal
        //    -inf / -normal
        result->value_type = s21_infinity;
    } else if ((divident.value_type == s21_infinity && divsr_is_normal_minus) ||
               (divident.value_type == s21_neg_infinity && divsr_is_normal_plus)) {
        //    +inf / -normal = -inf
        //    -inf / +normal
        result->value_type = s21_neg_infinity;
...
int s21_mod(s21_decimal a, s21_decimal b, s21_decimal *res) {
    s21_decimal tmp, zero = {{0, 0, 0, 0}, 0};
    int ret = 0;

    s21_set0bitstype(&tmp);  //  на всякий случай?
    s21_div(a, b, &tmp);
    if ((tmp.value_type == s21_infinity) || (tmp.value_type == s21_neg_infinity)) {
        *res = zero;
    } else {
    ...
    if (((a.value_type == s21_infinity) || (a.value_type == s21_neg_infinity)) && (b.value_type != s21_nan))
        res->value_type = s21_nan;

    if (((b.value_type == s21_infinity) || (b.value_type == s21_neg_infinity)) &&
    ...
    *result = value;
    if (result->value_type != s21_nan) {
        ...
        if (result->value_type == s21_infinity) {
            ...
            if (result->value_type == s21_neg_infinity) result->value_type = s21_infinity;
        }
    } else {  // inf or nan
        ret = 1;
    }
